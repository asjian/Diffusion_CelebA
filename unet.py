...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, t_emb = None):
        ...
        h = self.conv1(self.activation(self.gn1(x)))
        ...
        h = self.conv2(self.activation(self.gn2(h)))
        h = self.dropout(h)
...
class UNet(nn.Module):
    def __init__(self, in_channels, base_channels = 64, channel_mults = [1,2,4,8], emb_dim = 256):
        ...
        self.attention_down = SelfAttention(num_heads = 8, d_model = 512, dropout = 0.0)
        self.attention_up = SelfAttention(num_heads = 8, d_model = 512, dropout = 0.0)
        self.final_conv = nn.Conv2d(curr_channels, in_channels, 3, 1, 1)
    
    def forward(self, x, t):
        t_emb = self.emb(t)
        x = self.first_conv(x)
        #x = self.activation(x) #we comment this out to use the norm->activation->conv reordering in the residual blocks

        res_storage = []
        for i in range(0, len(self.down_layers), 3):
            x = self.down_layers[i](x, t_emb)

            if x.shape[-1] == 16: #16x16 resolution level, time for self attention
                x = self.attention_down(x)
